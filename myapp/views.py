...
def dashboard(request):
    ...
    filtered_users = (
        Users.objects.filter(college_department=selected_college).count()
...
def get_scanned_percentages(request):
    ...
    if college_filter:
        scanned_images = ScannedImage.objects.filter(email_address__college_department=college_filter)
    else:
        scanned_images = ScannedImage.objects.all()
...
def get_table_data(request):
    ...
    registered_query = UnrecognizedImages.objects.all()
    if start_date and end_date:
...
def export_filtered_college_data(request):
    try:
        ...
        unrecognized_images = UnrecognizedImages.objects.filter(email_address__in=users)
        scanned_images = ScannedImage.objects.filter(email_address__in=users)
        ...
        for user in users:
            ...
            data.append({
                ...
                "status": "Suspended" if user.isSuspended else "Warned" if user.isWarned else "Blocked" if user.isArchived else "Good",
...
def get_summary_table_data(request):
    ...
    data = (
        ...
        .values('category')
        .annotate(
            scanned=Sum(filter=Q()),  
            registered=Sum(filter=Q()),
            total=Sum(), 
        )
    )
